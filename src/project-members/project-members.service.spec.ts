...
describe('ProjectMembersService', () => {
  ...
  const mockRealtime = {
    emitProjectMemberEvent: jest.fn(),
  ...
  it('requires project management access before adding a member', async () => {
    ...
    expect(mockRealtime.emitProjectMemberEvent).toHaveBeenCalledWith(
      4,
  ...
  it('does not allow the owner to be demoted', async () => {
    ...
    await expect(
      ...
      new ForbiddenException('Project owner must remain a manager'),
    );
  });
