...
export class TasksService {
  ...
  async create(createTaskDto: CreateTaskDto, requester: AuthenticatedUser) {
      ...
      updateTaskDto.assignedToId !== undefined &&
        existingTask.assignedToId !== updatedTask.assignedToId && {
          ...
          metadata: {
            ...
            to: updatedTask.assignedToId,
          },
        },
      updateTaskDto.priority !== undefined &&
        existingTask.priority !== updatedTask.priority && {
          type: TaskActivityType.PRIORITY_CHANGED,
          metadata: { from: existingTask.priority, to: updatedTask.priority },
        },
      updateTaskDto.dueDate !== undefined &&
        existingTask.dueDate?.toISOString() !==
          updatedTask.dueDate?.toISOString() && {
          type: TaskActivityType.DUE_DATE_CHANGED,
          metadata: { from: existingTask.dueDate, to: updatedTask.dueDate },
        },
  ...
  private async moveTask(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (task.status === moveTaskDto.status) {
        ...
        const position =
          'position' in moveTaskDto
            ? Math.min(moveTaskDto.position, Math.max(taskCount - 1, 0))
            : task.position;
      ...
      const position =
        'position' in moveTaskDto
          ? Math.min(moveTaskDto.position, targetColumnCount)
          : targetColumnCount;
